...
void setDefaultStartValues(ModelInstance *comp) {
  ...
  put_real_element(100.0, 0, &comp->fmuData->modelData->realParameterData[0].attribute.start);
  put_real_element(0.5, 0, &comp->fmuData->modelData->realParameterData[1].attribute.start);
  put_real_element(25.0, 0, &comp->fmuData->modelData->realParameterData[2].attribute.start);
}
// Set values for all variables that define a start value
OMC_DISABLE_OPT
...
fmi2Status setReal(ModelInstance* comp, const fmi2ValueReference vr, const fmi2Real value) {
  ...
  if (vr < 6 && (comp->state == model_state_instantiated || comp->state == model_state_initialization_mode)) {
